...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public interface GameHelperListener {
         ...
         * user interaction was required (consent
         * dialogs). So implementations of this method
         * should NOT display an error message unless a
         * call to @link{GamesHelper# hasSignInError}
    ...
    /*
     * If we have incoming requests when we connected to the games client, they are here. Otherwise,
     * it's null.
     */
     ...
     * maximum is over the lifetime of the application (it is stored in a SharedPreferences file).
     * So, for example, if you specify 2, then it means that the user will be prompted to sign in on
     * app startup the first time and, if they cancel, a second time the next time the app starts,
     * and, if they cancel that one, never again. Set to 0 if you do not want the user to be
     * prompted to sign in on application startup.
     */
     ...
     * this; use this method only if you need to make nonstandard setup (e.g. adding extra scopes
     * for other APIs) on the GoogleApiClient.Builder before calling @link{#setup}.
     */
    ...
    void notifyListener(boolean success) {
        debugLog("Notifying LISTENER of sign-in " + (success ? "SUCCESS" : this.mSignInFailureReason != null ? "FAILURE (error)" : "FAILURE (no error)"));
    ...
    /*
     * Starts a user-initiated sign-in flow. This should be called when the user clicks on a
    ...
    public void onConnected(Bundle connectionHint) {
        ...
        if (connectionHint != null) {
            ...
            this.mTurnBasedMatch = connectionHint.getParcelable(Multiplayer.EXTRA_TURN_BASED_MATCH);
        }
    ...
    int getSignInCancellations() {
        SharedPreferences sp = this.mAppContext.getSharedPreferences(this.GAMEHELPER_SHARED_PREFS, Context.MODE_PRIVATE);
    ...
    int incrementSignInCancellations() {
        ...
        SharedPreferences.Editor editor = this.mAppContext.getSharedPreferences(this.GAMEHELPER_SHARED_PREFS, Context.MODE_PRIVATE).edit();
    ...
    public static class SignInFailureReason {
        ...
        public String toString() {
            return "SignInFailureReason(serviceErrorCode:"
                    ...
                    + ((this.mActivityResultCode == NO_ACTIVITY_RESULT_CODE) ? ")" : (",activityResultCode:"
                    + GameHelperUtils.activityResponseCodeToString(this.mActivityResultCode) + ")"));
        }
    }
